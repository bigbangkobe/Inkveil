...
namespace Framework
{
    ...
    /// UI管理器
    /// </summary>
    public class UIManager : EventNode, ILoadUIListener
    {
        ...
        public void OnShowAnimationUI(Transform transform)
        {
        ...
        /// </summary>
        /// <param name="transform"></param>
        public void OnHieAnimationUI(Transform transform)
        {
        ...
        /// </summary>
        /// <param name="transform"></param>
        public void OnDoYoyoAnimationUI(Transform transform,float scale = 1.2f)
        {
            transform.DOScale(new Vector3(scale, scale, scale), 1f).SetLoops(-1, LoopType.Yoyo);
        }
        ...
        /// <returns></returns>
        public BaseUI GetBaseUI(string name)
        {
            return mDicUI[name];
        }
        ...
        public void CreateUI(string uiName, Type type, ILoadUIListener listener = null)
        {
            if(listener == null)
            {
                listener = this;
        ...
        public void ShowUI(string uiName, Type type, ILoadUIListener listener = null, object param = null, bool createCanCall = false)
        {
            if (listener == null)
            {
                listener = this;
        ...
        public class LoadResFinish : IResLoadListener
        {
            ...
            public void Finish(object asset,string name)
            {
                ...
                go.SetActive(false);
                BaseUI ui = go.GetComponent(cmd.type) as BaseUI;
                ui.UIInit();
                ui.UIName = cmd.uiName;
                go.gameObject.name = ui.UIName;
                ui.CacheTransform.SetParent(UIManager.Instance.UIROOT, false);
                UIManager.Instance.AddUI(ui);
                if (cmd.cmdType == Command.CmdType.CreateAndShow)
                {
                    UIManager.Instance.ShowUI(cmd.uiName, cmd.type, cmd.listener, cmd.param, cmd.createCanCall);
                }
